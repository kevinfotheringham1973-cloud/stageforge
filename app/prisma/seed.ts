...
async function seedBody(db: DbClient) {
  ...
  const meDeliverableDefsByStage: DeliverableDef[][] = [
    ...
    [
      ...
      { key: "del.common_programme_of_works", label: "Programme of Works", description: "Contractor's detailed programme of works, submitted alongside the SBAR/safety-group submission and PFI/NHS cost approval, ahead of the Pre-Contract Hold Point." },
  ...
  const waterDeliverableDefsByStage: DeliverableDef[][] = [
    ...
    [
      ...
      { key: "del.water_isolation_plan", section: "Technical Design Package", label: "Water Isolation Plan (zones/outlets affected, isolation sequence, continuity-of-supply arrangements for unaffected areas)", description: "SHTM 04-01 Part B/D — the forward plan for how shutdowns will be sequenced, distinct from the Permit-to-Work/Isolation Certificates in Gate 5 that evidence it was actually done." },
      { key: "del.water_flushing_plan", section: "Technical Design Package", label: "Flushing Plan (daily vs periodic/scheduled flushing regime for affected outlets and dead legs, during works and pending clearance)", description: "SHTM 04-01 Part D, HSG274 — the regime document, distinct from the jointing/flushing/disinfection records in Gate 5 that evidence it was carried out." },
      { key: "del.water_pou_filter_strategy", section: "Technical Design Package", label: "Point-of-use (POU) filter strategy for at-risk outlets pending clearance sampling", description: "SHTM 04-01 Part D, HSG274 — which outlets need interim POU filtration while the system stabilises after works, and the criteria for removing them once clearance sampling passes." },
      { key: "del.common_pre_construction_information", label: "Pre-Construction Information (CDM)", description: "CDM 2015." },
      { key: "del.common_construction_phase_plan", label: "Construction Phase Plan", description: "CDM 2015 reg 12 — produced by the Principal Contractor (or the contractor, if only one) before the construction phase begins." },
    ...
    [
      ...
      { key: "del.water_pou_filter_installation_records", label: "Point-of-use (POU) filter installation and change-out records for outlets under temporary mitigation", description: "Critical interim Legionella control step per SHTM 04-01 Part D — cannot be bypassed at PM level.", bypassAuthority: "AUTHORISED_PERSON_WATER" },
  ...
  await createDeliverableTemplates(roomOrWardRefreshStageTemplates, [
    ...
    [
      ...
      { key: "del.wardrefresh_water_isolation_plan", label: "Water Isolation Plan for affected ensuite/ward outlets (isolation sequence, continuity-of-supply arrangements for unaffected areas)", description: "SHTM 04-01 Part B/D — required wherever ensuite or ward pipework/outlets are disturbed, e.g. ensuite upgrades." },
      { key: "del.wardrefresh_flushing_plan", label: "Flushing Plan for affected outlets (daily vs periodic/scheduled flushing regime during works and pending clearance)", description: "SHTM 04-01 Part D, HSG274 — disturbed or newly dead-legged pipework needs a defined flushing regime, not just a one-off flush." },
    ...
    [
      ...
      { key: "del.wardrefresh_water_isolation_flushing_records", label: "Water isolation, flushing and disinfection records for disturbed ensuite/ward pipework", description: "Critical Legionella control step, per SHTM 04-01 Part D — cannot be bypassed at PM level.", bypassAuthority: "AUTHORISED_PERSON_WATER" },
      { key: "del.wardrefresh_pou_filter_install", label: "Installation of point-of-use (POU) filters on affected outlets pending clearance sampling", description: "Critical interim Legionella control step per SHTM 04-01 Part D — cannot be bypassed at PM level.", bypassAuthority: "AUTHORISED_PERSON_WATER" },
      { key: "del.common_progress_records_quality_log", label: "Progress records, quality inspections, change control log" },
  ...
  await createDeliverableTemplates(mhuLigatureStageTemplates, [
    ...
    [
      ...
      { key: "del.common_updated_cost_plan_contingency", label: "Updated cost plan including appropriate contingency", description: "A realistic budget envelope for the PFI Board / NHS lifecycle cost approval — a condition for the Pre-Contract Hold Point below." },
      { key: "del.common_mep_interface_design", label: "Interface design with ventilation, medical gases, electrical, nurse call, drainage and fire systems", description: "SHTM 03-01 (ventilation), SHTM 02-01 (medical gas), SHTM 06 series (electrical), SHTM 08-03 (nurse call), SHTM 04-01 (drainage/water)." },
      { key: "del.mhu_water_isolation_plan", label: "Water Isolation Plan for affected ensuite outlets (isolation sequence, continuity-of-supply arrangements for unaffected areas)", description: "SHTM 04-01 Part B/D — required wherever ensuite fittings/pipework are disturbed as part of ligature works." },
      { key: "del.mhu_flushing_plan", label: "Flushing Plan for affected outlets (daily vs periodic/scheduled flushing regime during works and pending clearance)", description: "SHTM 04-01 Part D, HSG274 — disturbed or newly dead-legged pipework needs a defined flushing regime, not just a one-off flush." },
      { key: "del.common_infection_control_design_review", label: "HAI-SCRIBE Stage 2 design review and infection control measures", description: "HAI-SCRIBE Stage 2.", bypassAuthority: "COMPLIANCE_OFFICER" },
      { key: "del.common_fire_strategy_means_of_escape", label: "Fire strategy and means of escape implications", description: "Firecode.", bypassAuthority: "FIRE_OFFICER" },
      { key: "del.common_building_regs_compliance_info", label: "Building Warrant / Building (Scotland) Regulations 2004 compliance information (Schedule 3 where the works affect an existing occupied building)", bypassAuthority: "COMPLIANCE_OFFICER" },
      { key: "del.common_pre_construction_information", label: "Pre-Construction Information (CDM)", description: "CDM 2015." },
    ...
    [
      ...
      { key: "del.common_infection_control_construction_measures", label: "Dust, water and infection control measures during construction", description: "HAI-SCRIBE Stage 3.", bypassAuthority: "COMPLIANCE_OFFICER" },
      { key: "del.mhu_water_isolation_flushing_records", label: "Water isolation, flushing and disinfection records for disturbed ensuite pipework", description: "Critical Legionella control step, per SHTM 04-01 Part D — cannot be bypassed at PM level.", bypassAuthority: "AUTHORISED_PERSON_WATER" },
      { key: "del.mhu_pou_filter_install", label: "Installation of point-of-use (POU) filters on affected outlets pending clearance sampling", description: "Critical interim Legionella control step per SHTM 04-01 Part D — cannot be bypassed at PM level.", bypassAuthority: "AUTHORISED_PERSON_WATER" },
      { key: "del.common_progress_records_quality_log", label: "Progress records, quality inspections, change control log" },
  ...
  const theatreRefreshTemplate = await db.template.create({
    data: {
      ...
      description:
        "Refurbishment or refresh of an operating theatre — finishes, wall/ceiling cladding, doors, and fixed furniture, plus validation of any disturbed ventilation/medical gas/electrical/scrub-sink interfaces, in an operational healthcare environment. For a general (non-theatre) ward/room refresh see Room or Ward Refresh; for Mental Health Unit ligature work see Mental Health Unit Ligature & Room Refresh. Not a plant or M&E system replacement — see the relevant system template for that.",
      matchKeywords: [
        "theatre refresh",
  ...
  await createDeliverableTemplates(theatreRefreshStageTemplates, [
    ...
    [
      ...
      { key: "del.common_updated_cost_plan_contingency", label: "Updated cost plan including appropriate contingency", description: "A realistic budget envelope for the PFI Board / NHS lifecycle cost approval — a condition for the Pre-Contract Hold Point below." },
      { key: "del.common_mep_interface_design", label: "Interface design with ventilation, medical gases, electrical, nurse call, drainage and fire systems", description: "SHTM 03-01 (ventilation), SHTM 02-01 (medical gas), SHTM 06 series (electrical), SHTM 08-03 (nurse call), SHTM 04-01 (drainage/water)." },
      { key: "del.theatrerefresh_water_isolation_plan", label: "Water Isolation Plan for affected scrub sinks/theatre outlets (isolation sequence, continuity-of-supply arrangements for unaffected theatres)", description: "SHTM 04-01 Part B/D — required wherever scrub sink or theatre wet-service pipework/outlets are disturbed." },
      { key: "del.theatrerefresh_flushing_plan", label: "Flushing Plan for affected scrub sink/theatre outlets (daily vs periodic/scheduled flushing regime during works and pending clearance)", description: "SHTM 04-01 Part D, HSG274 — augmented-care outlets need a defined flushing regime, not just a one-off flush." },
      { key: "del.common_infection_control_design_review", label: "HAI-SCRIBE Stage 2 design review and infection control measures", description: "HAI-SCRIBE Stage 2.", bypassAuthority: "COMPLIANCE_OFFICER" },
      { key: "del.common_fire_strategy_means_of_escape", label: "Fire strategy and means of escape implications", description: "Firecode.", bypassAuthority: "FIRE_OFFICER" },
      { key: "del.common_building_regs_compliance_info", label: "Building Warrant / Building (Scotland) Regulations 2004 compliance information (Schedule 3 where the works affect an existing occupied building)", bypassAuthority: "COMPLIANCE_OFFICER" },
      { key: "del.common_pre_construction_information", label: "Pre-Construction Information (CDM)", description: "CDM 2015." },
    ...
    [
      ...
      { key: "del.common_infection_control_construction_measures", label: "Dust, water and infection control measures during construction", description: "HAI-SCRIBE Stage 3.", bypassAuthority: "COMPLIANCE_OFFICER" },
      { key: "del.theatrerefresh_water_isolation_flushing_records", label: "Water isolation, flushing and disinfection records for disturbed scrub sink/theatre pipework", description: "Critical Legionella control step, per SHTM 04-01 Part D — cannot be bypassed at PM level.", bypassAuthority: "AUTHORISED_PERSON_WATER" },
      { key: "del.theatrerefresh_pou_filter_install", label: "Installation of point-of-use (POU) filters on affected scrub sink/theatre outlets pending clearance sampling", description: "Critical interim Legionella control step per SHTM 04-01 Part D — cannot be bypassed at PM level.", bypassAuthority: "AUTHORISED_PERSON_WATER" },
      { key: "del.common_progress_records_quality_log", label: "Progress records, quality inspections, change control log" },
  ...
  await createDeliverableTemplates(waterTreatmentStageTemplates, [
    ...
    [
      ...
      { key: "del.watertreatment_isolation_plan", section: "Technical Design Package", label: "Water Isolation Plan for the treatment plant and distribution loop (isolation sequence, continuity arrangements for dialysis/pharmacy/CSSD end uses during works)", description: "SHTM 04-01 Part B/D." },
      { key: "del.watertreatment_flushing_plan", section: "Technical Design Package", label: "Flushing Plan for the distribution loop (daily vs periodic/scheduled flushing regime during works and pending validation)", description: "SHTM 04-01 Part D, HSG274, ISO 23500-3." },
      { key: "del.common_building_regs_compliance_info", label: "Building Warrant / Building (Scotland) Regulations 2004 compliance information (Schedule 3 where the works affect an existing occupied building)", bypassAuthority: "COMPLIANCE_OFFICER" },
      { key: "del.common_pre_construction_information", label: "Pre-Construction Information (CDM)", description: "CDM 2015." },
      { key: "del.common_construction_phase_plan", label: "Construction Phase Plan", description: "CDM 2015 reg 12 — produced by the Principal Contractor (or the contractor, if only one) before the construction phase begins." },
      { key: "del.common_design_risk_assessment_signed", label: "Designer's Risk Assessment & residual risks", description: "CDM 2015.", bypassAuthority: "SRO" },
    ...
    [
      ...
      { key: "del.watertreatment_permit_to_work_isolation", label: "Permit-to-Work and Isolation Certificates for every plant/loop shutdown", bypassAuthority: "AUTHORISED_PERSON_ELECTRICAL" },
      { key: "del.watertreatment_pou_filter_install", label: "Installation of point-of-use (POU) filters at affected dialysis/pharmacy/CSSD stations pending validation", description: "Critical interim Legionella/water-quality control step per SHTM 04-01 Part D, ISO 23500 — cannot be bypassed at PM level.", bypassAuthority: "AUTHORISED_PERSON_WATER" },
