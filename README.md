...
| [`ProvisioningModel.html`](ProvisioningModel.html) | Scoping for AI-assisted project provisioning — template-library matching, the draft/review/activate flow (data model, actions & permissions), and the LLM call itself (structured-output enforcement, model/prompt-caching design). (draft v0.3, not built) |
